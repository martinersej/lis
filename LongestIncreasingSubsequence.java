...
public class LongestIncreasingSubsequence {
    public ArrayList<Integer> longestIncreasingSubsequence(ArrayList<Integer> input) {
        int[][] dataset = new int[input.size()][2];
        int max = 0;
        for(int i = 0; i < input.size(); ++i){
            dataset[i][0] = Integer.MIN_VALUE;
            dataset[i][1] = 1;
            for(int j = i-1; j >= 0; --j){
                if(input.get(i) > input.get(j)){
                    if(dataset[i][1] <= dataset[j][1] + 1){
                        dataset[i][1] = dataset[j][1] + 1;
                        dataset[i][0] = j;
                    }
                }
            }
            max = Math.max(max,dataset[i][1]);
        }
        ArrayList<Integer> result = new ArrayList<>();
        for(int i = 0; i < input.size(); ++i){
            if(dataset[i][1] == max){
                int now = i;
                while(now != Integer.MIN_VALUE){
                    result.add(input.get(now));
                    now = dataset[now][0];
                }                
                break;
            }
